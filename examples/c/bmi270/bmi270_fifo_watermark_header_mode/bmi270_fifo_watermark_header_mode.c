 ...
 *
 * @file  bmi270_fifo_watermark_header_mode.c
 *
 */
 ...
 * @brief example to showcase FIFO watermark level for accel and gyro data in header mode
...
int main(void)
{
     ...
     * 55 * (6 bytes(2 for each axis)) = 330 bytes */
    ...
    /* Variable to get water-mark level. */
            ...
            if (rslt == BMI2_OK)
            {
                ...
                if (rslt == BMI2_W_FIFO_EMPTY)
                {
                    ...
                    if (rslt == BMI2_W_FIFO_EMPTY)
                    {
                        ...
                        printf("Sensor time = %u", fifoframe.sensor_time);
                    }
                }
            }
