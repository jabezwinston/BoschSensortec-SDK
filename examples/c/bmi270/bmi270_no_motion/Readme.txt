Overview
========
This example demonstrates no-motion interrupt.
Accel is enabled and the map interrupt for no-motion is set. The no-motion status is read and the interrupt occurs when the threshold is not exceeded.
...
6.  No-motion feature is chosen and the threshold is set.
7. 	The interrupt is mapped and the status is continously read.
8.	Interrupt occurs when the threshold is not exceeded.
