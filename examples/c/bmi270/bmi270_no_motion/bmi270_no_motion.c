 ...
 *
 * @file    bmi270_no_motion.c
 *
 */

/*!
 * @ingroup bmi2xyGroupExample
 * @defgroup bmi2xyGroupExample No Motion
 * @brief example to showcase No motion interrupt output
 */
...
int main(void)
{
    ...
    /* Variable to get no-motion status */
    ...
    if (rslt == BMI2_OK)
    {
        ...
        if (rslt == BMI2_OK)
        {
            printf("Do not move the board for no motion interrupt\n");

            /* Loop to print the no motion interrupt data when interrupt occurs */
            while (1)
            {
                /* Clear buffer */
                int_status = 0;

                /* Get the interrupt status of the no-motion */
                rslt = bmi2_get_int_status(&int_status, &bmi2);
                print_rslt(rslt);

                /* Check the interrupt status of the no-motion */
                if ((rslt == BMI2_OK) && (int_status & BMI270_NO_MOT_STATUS_MASK))
                {
...
static int8_t bmi2_set_config(struct bmi2_dev *bmi2_dev)
{
    ...
    if (rslt == BMI2_OK)
    {
        ...
        if (rslt == BMI2_OK)
        {
            /* The following configuration parameter values may change according to the user requirement */
