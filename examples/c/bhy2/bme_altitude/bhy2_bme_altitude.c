...
#define BHY_SENSOR_ID_ALTITUDE       0x96
...
/*!
 * @brief function to parse altitude frame 
 *
 *
 * @param[in] p_fifo_buffer: pointer to fifo buffer
 * @param[in] p_bhy2_hub: pointer to hub
 * @param[out] none.
 *
 * @return res: operation result
 */
s16 bhy2_parse_frame_altitude(fifo_buffer_t* p_fifo_buffer, bhy2_hub_t* p_bhy2_hub)
{

    float tmp_time_stamp = 0;
    s32 tmp_altitude = 0;
 
    float tmp_scale_factor = (float)1 / 100;
    s32 ts32_read_pos = p_fifo_buffer->read_pos;

    tmp_altitude = p_fifo_buffer->p_buffer[(ts32_read_pos + 1)]
    ...
    p_bhy2_hub->virt_sensor[BHY_SENSOR_ID_ALTITUDE].data.scalar_u32.data_u32 = tmp_altitude;
    tmp_time_stamp = (float)p_bhy2_hub->time_stamp / 64000;
    p_fifo_buffer->read_pos += 5;

    TST_PRINT("sensor: altitude timestamp: %13.6fs  Altitude: %3fm\r\n", tmp_time_stamp, (float)tmp_altitude * tmp_scale_factor);
...
{
    bhy2_virt_sensor_callback_register( p_bhy2_hub, BHY_SENSOR_ID_ALTITUDE, bhy2_parse_frame_altitude );
...
int main(void)
{
    ...
    ASSERT ( bhy2_virt_sensor_cfg_set(&bhy2_hub_obj,BHY_SENSOR_ID_ALTITUDE,sample_rate, 0) );

    TST_PRINT("enable sensor as %f HZ\r\n",sample_rate); 
