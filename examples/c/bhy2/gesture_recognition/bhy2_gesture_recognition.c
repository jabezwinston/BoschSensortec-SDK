 ...
 *
 * @file    bhy2_gesture_recognition.c
 * @date    Dec-10-2018
 * @version 1.1
 * @brief   Sample file how to test gesture recognition capability of BHY2 
 *          using LIB COINES
 *
 */
/*********************************************************************/
...
#ifdef BHI260
#include "Bosch_SHUTTLE_BHI260_AK09915.h"
#endif

#ifdef BHA260
#include "Bosch_SHUTTLE_BHA260_AK09915.h"
#endif
...
{
    bhy2_virt_sensor_callback_register( p_bhy2_hub, BHY_SENSOR_ID_WAKE_WU, bhy2_parse_frame_wake_gesture_wkup);
    bhy2_virt_sensor_callback_register( p_bhy2_hub, BHY_SENSOR_ID_GLANCE_WU, bhy2_parse_frame_glance_gesture_wkup);
    bhy2_virt_sensor_callback_register( p_bhy2_hub, BHY_SENSOR_ID_PICKUP_WU, bhy2_parse_frame_pickup_gesture_wkup );
    bhy2_virt_sensor_callback_register( p_bhy2_hub, BHY_SENSOR_ID_TILT_WU, bhy2_parse_frame_tilt_detect_wkup );
...
int main(void)
{
    ...
    if((tu8_data & BHY_BST_HOST_INTERFACE_READY) == BHY_BST_HOST_INTERFACE_READY)
    {
        ...
        if((tu8_data & BHY_BST_FLASH_DETECTED) == BHY_BST_FLASH_DETECTED)
        {
            ...
            else//no patch ext-in flash
            {
                TST_PRINT ( "No patch is in ext-flash, need to write patch to ext-flash.\r\n");
            }
        }
        else//without ext-flash, need to upload ram patch
        {
    ...
    ASSERT ( bhy2_virt_sensor_cfg_set(&bhy2_hub_obj,BHY_SENSOR_ID_TILT_WU,sample_rate, 0) );
    TST_PRINT("enabled Tilt sensor");
	fflush(stdout);
