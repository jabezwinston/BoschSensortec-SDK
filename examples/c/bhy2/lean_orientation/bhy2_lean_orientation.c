 ...
 *
 * @file    bhy2_lean_orientation.c
 * @date    Dec-10-2018
 * @version 1.1
 * @brief   Sample file how to display altitude using BHY2 
 *          with LIB COINES
 *
 */
/*********************************************************************/
...
/* macro definitions */
#ifdef BHI260
#include "Bosch_SHUTTLE_BHI260_AK09915_Cus.h"
#endif

#ifdef BHA260
#include "Bosch_SHUTTLE_BHA260_AK09915_Cus.h"
#endif

#define BHA260_SHUTTLE_ID 0x139
#define BHI260_SHUTTLE_ID 0x119
#define VDD_VDDIO_VALUE   3300
#define BHY_SENSOR_ID_LEAN_ORIENTATION SENSOR_TYPE_CUSTOMER_VISIBLE_START /*Lean orientation sensor ID*/
...
/*!
 * @brief parse lean_orientation frame
 *
 *
 ...
 */
s16 bhy2_parse_frame_lean_orientation(fifo_buffer_t* p_fifo_buffer, bhy2_hub_t* p_bhy2_hub)
{
    ...
    tmp_lean[0] = p_fifo_buffer->p_buffer[(ts32_read_pos + 2)];// +/-
    tmp_lean[1] = p_fifo_buffer->p_buffer[(ts32_read_pos + 1)];

    tmp_time_stamp = (float)p_bhy2_hub->time_stamp / 64000; /* 1 LSB = 1/64000 sec */
    p_fifo_buffer->read_pos += 3;

    TST_PRINT("sensor: lean_orientation timestamp: %13.6fs Lean_orientation %s \r\n", tmp_time_stamp, tmp_lean);
...
{
    bhy2_virt_sensor_callback_register( p_bhy2_hub, BHY_SENSOR_ID_LEAN_ORIENTATION, bhy2_parse_frame_lean_orientation );
...
int main(void)
{
    ...
    ASSERT ( bhy2_virt_sensor_cfg_set(&bhy2_hub_obj,BHY_SENSOR_ID_LEAN_ORIENTATION,sample_rate, 0) );

    TST_PRINT("enable cus LEAN ORIENTATION sensor as %f HZ\r\n",sample_rate); 
	fflush(stdout);
