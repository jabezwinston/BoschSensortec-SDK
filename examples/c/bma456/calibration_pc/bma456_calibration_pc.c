 ...
 *
 * @file    bma456_calibration.c
 * @brief 	Sample file to calibrate BMA456 offset using LIB COINES
 *
 */
...
/*!
 * @brief	This internal API is used to initialize the bma456 and calibrate offset
 */
static void bma456_calibration(void);
 ...
 */
static void bma456_get_offset(double *, double *, double *);
...
/*! This internal API quits the program and outputs an error message */
 ...
 *
 */
static void bma456_get_offset(double *x_off_mg, double *y_off_mg,
                              double *z_off_mg)
{
    uint16_t commrslt;
    ...
    for (int i = 0; i < 20; ++i)
    {
        ...
        /* ( 'datapoint' * '2G' * '2' * '1000mg') / ('scaling factor for 16 bits') */
        xval = (((double)sens_data[i].x) * 2. * 2. * 1000) / pow(2, 16);
        yval = (((double)sens_data[i].y) * 2. * 2. * 1000) / pow(2, 16);
        zval = (((double)sens_data[i].z) * 2. * 2. * 1000) / pow(2, 16);
...
static void bma456_update_nvm(void)
{
    ...
    do
    {
        commrslt = bma4_read_regs(BMA4_STATUS_ADDR, &data, 1, &bma456dev);
        if (0 != commrslt)
            coines_exit_error("Unknown communication error. Exiting...\n");
    }
    while (0 == (data & 0x10));
    ...
    commrslt = bma4_set_command_register(0xA0, &bma456dev);
    if (0 != commrslt)
        coines_exit_error("Unknown communication error. Exiting...\n");
...
static void bma456_calibration(void)
{
    ...
    bma456_get_offset(&x_off_mg, &y_off_mg, &z_off_mg);
    ...
    commrslt = bma4_perform_accel_foc(target_values, &bma456dev);
