...
ifeq ($(TARGET),$(filter $(TARGET),MCU_APP20 MCU_APP30))
    CFLAGS += -std=c99 -mthumb -mabi=aapcs -mcpu=cortex-m4 -c -Og -g3 -Wall \
	          -Dprintf=iprintf -Dscanf=iscanf -D$(TARGET)
    CPPFLAGS += -mthumb -mabi=aapcs -mcpu=cortex-m4 -c -Og -g3 -Wall -D$(TARGET)
...
ifeq ($(TARGET),PC)
CFLAGS += -std=gnu99 -c -g3 -O0 -D$(TARGET) -Wall
CPPFLAGS += -c -g3 -O0 -D$(TARGET) -Wall
CROSS_COMPILE =
endif
...
ifeq ($(OS),Windows_NT)
    ...
    LIB_PATH ?= $(COINES_INSTALL_PATH)/lib
...
INCLUDEPATHS += $(COINES_INSTALL_PATH)/coinesAPI

LIBPATHS += \
$(LIB_PATH_ARCH) \
$(COINES_INSTALL_PATH)/coinesAPI \
...
ifeq ($(TARGET),MCU_APP20)
    DEVICE = APP2.0-DFU
        ...
        ifeq ($(LOCATION),RAM)
            LD_SCRIPT = $(COINES_INSTALL_PATH)/coinesAPI/mcu_app20/mcu_app20_ram.ld
        else
            LD_SCRIPT = $(COINES_INSTALL_PATH)/coinesAPI/mcu_app20/mcu_app20_flash.ld
        endif
...
ifeq ($(TARGET),MCU_APP30)
    DEVICE = Nuwa-DFU
        ifeq ($(LOCATION),RAM)
            LD_SCRIPT = $(COINES_INSTALL_PATH)/coinesAPI/mcu_app30/linker_scripts/mcu_app30_ram.ld
        else
            LD_SCRIPT = $(COINES_INSTALL_PATH)/coinesAPI/mcu_app30/linker_scripts/mcu_app30_flash.ld
        endif
...
$(EXE): $(OBJ_DIR) $(C_OBJS) $(CPP_OBJS) $(ASM_OBJS)
	@echo [ MAKE ] coinesAPI
	@$(MAKE) -s -C  $(COINES_INSTALL_PATH)/coinesAPI TARGET=$(TARGET)
	@echo [ LD ] $@
...
clean-all: clean
	...
	@$(MAKE) -s -C  $(COINES_INSTALL_PATH)/coinesAPI clean
